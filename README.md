...
- `Complex c_div(Complex a, Complex b)` — divides two complex numbers
...
- `longComplex c_sub(longComplex a, longComplex b)` — subtracts two long complex numbers
- `longComplex c_mult(longComplex a, longComplex b)` — multiplies two long complex numbers
- `longComplex c_div(longComplex a, longComplex b)` — divides two long complex numbers
---
...
### Methods
- `double v_norm(Vector3 u)` — returns the norm of a 3D Vector
- `double v_dot(Vector3 u, Vector3 v)` — returns the dot product of two 3D Vectors
- `Vector3 v_add(Vector3 u, Vector3 v)` — adds two 3D Vectors
- `Vector3 v_sub(Vector3 u, Vector3 v)` — subtracts two 3D Vectors
- `Vector3 v_prod(Vector3 u,Vector3 v)` — returns the vectorial product of two 3D Vectors
- `Vector3 v_scale(Vector3 u, double k)` — scales a 3D Vector by a real factor
---
...
### Methods
- `long double v_norm(longVector3 u)` — returns the norm of a long 3D Vector
- `long double v_dot(longVector3 u, longVector3 v)` — returns the dot product of two long 3D Vectors
- `longVector3 v_add(longVector3 u, longVector3 v)` — adds two long 3D Vectors
- `longVector3 v_sub(longVector3 u, longVector3 v)` — subtracts two long 3D Vectors
- `longVector3 v_prod(longVector3 u,longVector3 v)` — returns the vectorial product of two long 3D Vectors
- `longVector3 v_scale(longVector3 u, long double k)` — scales a long 3D Vector by a real factor
---
